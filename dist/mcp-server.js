...
export class MCPServer {
    ...
    formatForLLM(response, options) {
        ...
        const filteredResults = options.relevanceThreshold
            ? results.filter((r) => r.relevance >= (options.relevanceThreshold || 0))
            : results;
        ...
        if (options.includeSummary && filteredResults.length > 0) {
            summary = `Found ${filteredResults.length} relevant results for "${response.query}". ` +
                `Top sources include: ${filteredResults.slice(0, 3).map((r) => r.domain).join(', ')}. ` +
    ...
    calculateRelevance(result, query) {
        ...
        if (titleLower.includes(queryLower))
            score += 0.4;
        if (titleLower.includes(queryLower.split(' ')[0]))
            score += 0.2;
        // Snippet relevance
        if (snippetLower.includes(queryLower))
            score += 0.3;
        if (snippetLower.includes(queryLower.split(' ')[0]))
            score += 0.1;
    ...
    calculateConfidence(results, processingTime) {
        if (results.length === 0)
            return 0;
